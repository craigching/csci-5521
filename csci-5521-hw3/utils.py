...
def wrapper_help(name, datasets, methods):
    ...
    print(('python ' + name + '.py'
           ...
           ' -m <{}|{}>'
           ' -k [num folds]'
           ' -l [latex output on]'.format(methods[0], methods[1])))


def wrapper_args(argv, name, datasets, methods):
    ...
    if argv:
        try:
            ...
            wrapper_help(name, datasets)
            sys.exit()

        for opt, arg in opts:
            if opt == '-h':
                wrapper_help(name, datasets, methods)
                sys.exit(2)
            elif opt == '-l':
                latex_output = True
            elif opt == '-m':
